...
3. Build the signed release into a new `dist\vVERSION` directory.
4. Verify the timestamp, signature status, signer identity, and SHA-256 checksum.
...
7. Test that exact signed executable on the Sophos-protected endpoint under the intended policy. Run both Create and Remove, then check local events and Sophos Central. If Sophos still blocks intended behavior, retain the event details and submit the exact sample/hash for false-positive review. Do not disable protection or add a broad exclusion.
8. Upload both `AgentSshKeyManager.exe` and `AgentSshKeyManager.exe.sha256` from the version directory. Download them again and repeat the verification before announcing the release.
